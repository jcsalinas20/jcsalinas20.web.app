...
function Projects() {
  ...
  useEffect(() => {
    async function fetchData() {
      setBasicRepos(
        await fetch(`${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_GIT_USER}/repos/basic`, {
      ...
      if (!error503) {
        setRepos(
          await fetch(`${process.env.REACT_APP_API_DOMAIN}/${process.env.REACT_APP_GIT_USER}/repos`, {
            headers: { Authorization: `${process.env.REACT_APP_JWT_API}` },
          })
            .then((res) => res.json())
            .then((json) => {
    ...
    <div className="my-projects" id="projects">
      <Header as="h1">
        ...
        <div className="filter">
          ...
          />
          {selectedFilterType === "type" || selectedFilterType === "lang" || selectedFilterType === "topics" ? (
      ...
      <Segment color="blue" className="projects">
        ...
        <div className="background-particles">
          ...
          {searcher.length === 0 ? (
            <>
              <ProjectCard key={0} placeholder={true} />
              <ProjectCard key={1} placeholder={true} />
              <ProjectCard key={2} placeholder={true} />
              <ProjectCard key={3} placeholder={true} />
              <ProjectCard key={4} placeholder={true} />
              <ProjectCard key={5} placeholder={true} />
            </>
          ) : (
            searcher.map((repo, key) => {
              return <ProjectCard key={key} repo={repo} placeholder={false} />;
            })
          )}
        </div>
      </Segment>
